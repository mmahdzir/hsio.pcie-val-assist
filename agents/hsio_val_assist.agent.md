...
## 🗺️ Primary Workflow Reference

Before starting any task, refer to the visual workflow diagrams — these are your **primary structural reference**:

📊 **[Agent Workflow Structure Diagrams](../docs/hsio_val_agent_diagrams.html)**

The diagrams cover all 4 objectives with step-by-step swimlane flows:
- **Objective 1 · Create New Test** — Trigger → Model Detection → 3-input Knowledge Gathering → Code Generation → Build → Validate
- **Objective 2 · Debug a Failure** — Log Collection → Failure Classification → Root Cause Analysis → Fix & Verify
- **Objective 3 · Regression Report** — Regression Discovery → Data Collection → Report Generation → Publish

Always follow the swimlane order shown in the diagram. The 3 parallel knowledge inputs (Model Structure, Regression Examples, Agent Memory) must all be gathered before generating code or analysis.
...
### Test Ownership (All Users)
- **JSON source**: `testplan/all_tests.json` (generated by `testplan/gen_all_tests.py`)
- Contains ALL test owners: chewyeec, chinfong, jyeoh, mmahdzir, namlee, nmorshid
- **Per-user lookup**: Use `$USER` environment variable to filter tests for the current user
- Example: To find tests for current user in VAL0P8:
  ```bash
  python3 -c "
  import json, os
  data = json.load(open('testplan/all_tests.json'))
  user = os.environ.get('USER', 'mmahdzir')
  my_tests = data['by_owner'].get(user, [])
  val08 = [t for t in my_tests if 'VAL0P8' in t.get('soc_milestone','')]
  print(f'{user} has {len(val08)} tests in VAL0P8:')
  for t in val08: print(f'  {t["test_name"]}')
  "
  ```
- **Report format**: Rich HTML with charts and filters — `examples/ww<WW>_regression_report.html`
...
- File name: `ww<WW>_regression_report.html`
...
- All-user test JSON: `testplan/all_tests.json` — generated by `testplan/gen_all_tests.py`; use `$USER` for per-user filtering
...
> Never hardcode a specific username. Use `$USER` only for filtering against `testplan/all_tests.json`.
